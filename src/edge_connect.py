...
class EdgeConnect():
    def __init__(self, config):
        ...
        self.writer = SummaryWriter(log_dir='runs/edge')
            ...
            for items in train_loader:
                ...
                if model == 1:
                    ...
                    if iteration % self.config.LOG_INTERVAL == 0:
                        ...
                        with self.writer as writer:
                            writer.add_scalars('Train edge model(model=1)',{
                                'dis_loss' : dis_loss.item(), 'gen_loss' : gen_loss.item(),
                ...
                # inpaint model
                    ...
                    if iteration % self.config.LOG_INTERVAL == 0:
                        # zzoon : TensorboardX
                        with self.writer as writer:
                            writer.add_scalars('Train edge model(model=2)',{
                                'dis_loss' : dis_loss.item(), 'gen_loss' : gen_loss.item(),
                                'psnr' : psnr.item(),'mae' : mae.item()
                            }, iteration)
